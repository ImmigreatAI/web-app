...
export async function POST(request: NextRequest) {
  try {
    ...
    const bundleEnrollments = enrollments.filter(e => e.enrollment_type === 'bundle');

    // Create ownership map
    const bundleOwnershipMap = new Map<string, any>();
    bundleEnrollments.forEach(enrollment => {
    ...
    const userBundleData = bundleIds.map(bundleId => {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const allEnrollments = activeOnly ? 
      await purchaseService.getUserActiveEnrollments(userId) :
      await purchaseService.getUserAllEnrollments(userId);

    const bundleEnrollments = allEnrollments.filter(e => e.enrollment_type === 'bundle');

    // Get expiring bundle enrollments
    const expiringEnrollments = await purchaseService.getEnrollmentsExpiringSoon(userId, expiringDays);
    const expiringBundles = expiringEnrollments.filter(e => e.enrollment_type === 'bundle');

    // If includeCourses is true, also check which individual courses user owns
    let ownedCourseIds: string[] = [];
    if (includeCourses) {
      const courseEnrollments = allEnrollments.filter(e => e.enrollment_type === 'course');
      ownedCourseIds = courseEnrollments.map(e => e.item_id);
    ...
    const bundleData = bundleEnrollments.map(enrollment => ({
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        expiring_bundles: expiringBundles.map(enrollment => ({
          bundle_id: enrollment.item_id,
          bundle_title: enrollment.item_title,
          expires_at: enrollment.expires_at,
          days_until_expiry: Math.ceil((new Date(enrollment.expires_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24)),
      ...
      meta: {
        ...
        active_bundles: bundleData.filter(b => b.is_active).length,
